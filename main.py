...
from forpass import Ui_ForgotPassword
...
class EditData(QtWidgets.QDialog, Ui_edit_data):
    ...
    def opt_2(self):
        if self.comboBox_2.currentText() == 'Delete Specific Rows On Condition':
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def SaveCD(self):
        ...
        if not (len(df) == 0):
            ...
            msg.setText("Cannot Save Cleaned Data File")
            msg.setIcon(QMessageBox.Warning)
            msg.setDetailedText("You load any data to be saved.")
            msg.setWindowIcon(QtGui.QIcon('Images/Warning.png'))
            msg.exec_()

class forgotpass(QtWidgets.QDialog, Ui_ForgotPassword):
    def __init__(self, parent=None):
        super(forgotpass, self).__init__(parent)
        self.setupUi(self)
        self.Reset.setText("OK")
        self.Reset.clicked.connect(self.reseted)
        self.password.setEnabled(False); self.passwordlabel.setEnabled(False) 
        self.repassword.setEnabled(False); self.repasswordlabel.setEnabled(False)
    ...
    def reseted(self):
        ...
        if self.Reset.text() == "Reset":
            if self.password.text() == self.repassword.text():
                self.Reset.setText("OK")
                q.update_user(self.username.text(), 'password', self.password.text())
                widget.setFixedSize(510, 518)
                widget.setCurrentIndex(0)
            else:
                msg = QMessageBox()
                msg.setWindowTitle("CAUTION")
                msg.setText("!!! Password and Repassword doesn't match !!!")
                msg.setIcon(QMessageBox.Critical)
                msg.setWindowIcon(QtGui.QIcon('Images\\Critical.png'))
                msg.exec_()
        if self.Reset.text() == "OK":
            if q.check_user(self.username.text()) and str(q.get_user('username', self.username.text(), self.SecQuecomboBox.currentIndex())) == self.securityQue.text():
                self.Reset.setText("Reset")
                self.password.setEnabled(True); self.passwordlabel.setEnabled(True)
                self.repassword.setEnabled(True); self.repasswordlabel.setEnabled(True)
            else:
                msg = QMessageBox()
                msg.setWindowTitle("CAUTION")
...
class signup(QtWidgets.QDialog, Ui_signup):
    ...
    def created(self):
        global profileDetails
        q.connect()
        if not q.check_user(self.username.text()):
            if self.password.text() == self.repassword.text():
                if not(self.name.text() == " ") and not(self.username.text() == "") and not(self.SecQuecomboBox.currentIndex() == 0) and not(self.securityQue.text() == ""):
                    profileDetails = {'Type':'ProfileDetails',
                        'Name': self.name.text(),
                        'Username': self.username.text(),
                        'Password': self.password.text(),
                        self.SecQuecomboBox.currentText(): self.securityQue.text()
                    }
                    q.sign_up(profileDetails['Username'], profileDetails['Password'], self.SecQuecomboBox.currentIndex(), self.securityQue.text())
                    widget.setFixedSize(510, 518)
                    widget.setCurrentIndex(0)
                else:
                    msg = QMessageBox()
                    msg.setWindowTitle("CAUTION")
                    msg.setText("!!! Field(s) Cannot be Empty !!!")
                    msg.setIcon(QMessageBox.Critical)
                    msg.setWindowIcon(QtGui.QIcon('Images\\Critical.png'))
                    msg.exec_()
            else:
...
class login(QtWidgets.QDialog, Ui_LogIn):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.login.clicked.connect(self.mainWin)
        self.SignUp.clicked.connect(self.sign)
        self.Forget.clicked.connect(self.forgot)

    def sign(self):
        widget.setFixedSize(511, 621)
        widget.setCurrentIndex(1)
        
    def forgot(self):
        widget.setFixedSize(514, 503)
        widget.setCurrentIndex(2)

    def mainWin(self):
        q.connect()
        if q.login_user(self.username.text(), self.password.text()):
            widget.setFixedSize(843, 665)
            widget.setCurrentIndex(3) 
            self.username.clear()
            self.password.clear()
        else:
            msg = QMessageBox()
            msg.setWindowTitle("Invalid Username Or Password")
...
if __name__ == '__main__':
    ...
    widget.setWindowTitle("Data Analysis Tool")
    widget.setWindowIcon(QtGui.QIcon("Images\\1.png"))
    widget.addWidget(log)           # 0
    widget.addWidget(sign)          # 1
    widget.addWidget(forpass)       # 2
    widget.addWidget(mainwindow)    # 3
    widget.setFixedSize(510, 518)
    widget.show()
    sys.exit(app.exec_())